...
export default function WorkoutDetailedDescriptionComponent({
  ...
  const requiresDevice = workout.workoutTypeSequences.some(seq => seq.registerForce);
  ...
  const totalDuration = workout.workoutTypeSequences.reduce((sum, seq) => sum + seq.duration, 0);
    ...
    return (
        <div className="flex flex-col h-full w-full bg-base-100 rounded-xl shadow-lg p-6 overflow-hidden">
            {/* Header with title and description - unchanged */}
            <div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <div className="badge badge-outline">
                        {workout.workoutTypeSequences.length} sequences
                    </div>
                    {requiresDevice && (
                        <div className={`badge ${device ? 'badge-success' : 'badge-warning'}`}>
                            Force Tracking {device ? 'Connected' : 'Required'}
                        </div>
                    )}
                </div>
            </div>
                ...
                <div className="overflow-x-auto overflow-y-auto flex-grow">
                    <table className="table table-zebra w-full text-sm">
                        ...
                        <tr>
                            ...
                            <th className="w-16 text-center"><ClockIcon className="h-4 w-4 inline" /></th>
            ...
            <div className="mt-6 flex flex-col items-center">
                <button
                    ...
                    disabled={requiresDevice && !device}
