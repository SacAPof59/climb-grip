...
export default function CreateTimerPage() {
  ...
  const { copyTimerId } = useParams() as { copyTimerId?: string };
  ...
  // Fetch timer data if copying
  useEffect(() => {
    if (copyTimerId) {
      const fetchTimerData = async () => {
        setIsLoading(true);
        try {
          const response = await fetch(`/api/timer/${copyTimerId}`);
          if (!response.ok) {
            throw new Error('Failed to fetch timer data');
          }

          const timerData: Timer = await response.json();

          // Pre-fill the form with the timer data
          setTimerName(`${timerData.name} (Copy)`);

          // Check if it's a multi-step timer
          setIsMultiStepMode(timerData.steps.length > 1);

          // Map steps with new IDs
          const mappedSteps: TimerStep[] = timerData.steps.map((step: TimerStep) => ({
          ...
          alert('Failed to load timer template. Starting with empty form.');
        } finally {
          setIsLoading(false);
        }
      };
